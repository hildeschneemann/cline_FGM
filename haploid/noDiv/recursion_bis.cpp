			   ...
			   int T1v, int T2v, int T3v, int pasv, int nov)
{
	...
	int i, j, k, loc, gen, mut, chr1, chr2, ind, nb, nb1, nb2, nb3, nb4, nbMig, ns, part, nbCo, hs;
	...
	double* Wmean = new double[dv]; // mean fitness in each deme
	...
	for (i = 0; i < dv; i++) //go through demes
	{
		nb = i * Nv;
		hs = nbSv / 2;
		if (i < bv) //before barrier
			for (j =0; j< Nv; j++) //go through individuals
			{
				for (k =0; k < hs; k++) // flip first half of selected sites
				{	
					pop[j + nb].sel.flip(k);
					temp[j + nb].sel.flip(k);
				}
			}
		else 			
			for (j =0; j< Nv; j++) // go through individuals
			{
				for (k =hs; k < nbS_1; k++) //flip second half of selected sites
				{	
					pop[j + nb].sel.flip(k);
					temp[j + nb].sel.flip(k);
				}
